...
public class WallsBuilder : MonoBehaviour
{
    ...
    public void BuildWalls(Vector2Int mapSize)
    {
        for (int y = 0; y <= mapSize.y; y += _wallOffset)
        {
            if (y == mapSize.y)
            {
                Instantiate(_cornerWallLeft, new Vector3(-0.9f, 1, y - _sideOffset), _cornerWallLeft.transform.rotation, transform);
            }
            else
            {
                Instantiate(_straightWall, new Vector3(-0.9f, 1, y), _straightWall.transform.rotation, transform);
            }
        }
